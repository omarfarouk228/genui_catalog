...
- **`ActionFormWidget.dispatchEvent`** — signature changed from `void Function(String)` to
  `void Function(String, Map<String, String>)` so form field values are forwarded alongside
  the event name on submit.
