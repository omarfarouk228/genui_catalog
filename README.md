...
A catalog of **17 high-level UI components** (plus `Column` / `Row` layout containers) built on top of the [`genui`](https://pub.dev/packages/genui) SDK. Each component ships with its JSON schema, widget binding, event handling, dark/light mode support, and screen-reader semantics — ready to drop into any GenUI-powered app.
...
Register the catalog with your `SurfaceController`, then let `PromptBuilder` generate the system prompt from the catalog metadata, so the LLM always knows which components are available and what data each one expects.
...
// 2. Build the system prompt: your instructions + the A2UI protocol rules
//    + the JSON Schema of every component.
final prompt = PromptBuilder.chat(
  catalog: GenUICatalog.all,
  systemPromptFragments: ['You are a dashboard assistant.'],
);
final systemInstruction = prompt.systemPromptJoined(); // send to your LLM

// 3. Render a surface generated by the LLM
Surface(surfaceContext: controller.contextFor(surfaceId));
```

To connect an LLM, wrap it in an `A2uiTransportAdapter` (call your model in `onSend`, push the streamed text back with `addChunk`) and join both with a `Conversation`. The [`/example`](example/) app shows a complete Gemini integration in [`ai_service.dart`](example/lib/services/ai_service.dart).
...
SurfaceController(
  catalogs: [
    DataCatalog.asCatalog(),      // KpiCard, DataTable, ChartCard, StatRow, ListCard, EmptyState, Column, Row
...
Every interaction is dispatched as a `UserActionEvent`. The `SurfaceController` turns it into a `ChatMessage` on `onSubmit` — a `Conversation` forwards it to the LLM automatically. To react in your own code as well, decode it and compare the name with the `CatalogEvents` constants:

```dart
import 'dart:convert';

controller.onSubmit.listen((message) {
  for (final part in message.parts.whereType<DataPart>()) {
    if (part.mimeType != UiPartConstants.interactionMimeType) continue;
    final payload = jsonDecode(UiInteractionPart.fromDataPart(part).interaction);
    final action = payload['action'] as Map<String, dynamic>?;
    if (action == null) continue; // a validation error report, not a user action
...
See the [`/example`](example/) folder for a full demo app showcasing all 17 components with live Gemini generation.
