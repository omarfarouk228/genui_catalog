...
void main() {
  group('EmptyStateWidget', () {
    ...
    testWidgets('no action button when actionLabel is null', (tester) async {
      await tester.pumpWidget(
        wrap(EmptyStateWidget(title: 'Empty', dispatchEvent: (_) {})),
      );
      expect(find.byType(FilledButton), findsNothing);
    });

    testWidgets('renders without any text', (tester) async {
      await tester.pumpWidget(wrap(EmptyStateWidget(dispatchEvent: (_) {})));
      expect(find.byType(EmptyStateWidget), findsOneWidget);
    });

    testWidgets('renders custom icon name', (tester) async {
      await tester.pumpWidget(
        wrap(EmptyStateWidget(iconName: 'inbox', dispatchEvent: (_) {})),
      );
