...
void main() {
  ...
  group('DataTableWidget', () {
    ...
    testWidgets('shows truncation indicator when totalRowCount > rows.length', (tester) async {
      await tester.pumpWidget(wrap(
        DataTableWidget(
          columns: columns,
    ...
    testWidgets('does not show truncation indicator when totalRowCount is null', (tester) async {
      await tester.pumpWidget(wrap(
        DataTableWidget(columns: columns, rows: [{'name': 'Alice', 'age': '30'}]),
      ));
      expect(find.textContaining('Showing'), findsNothing);
    });

    testWidgets('renders with empty rows', (tester) async {
      await tester.pumpWidget(wrap(DataTableWidget(columns: columns, rows: const [])));
  ...
  group('dataTableItem truncation', () {
    testWidgets('shows indicator for > 100 rows', (tester) async {
      final manyRows = List.generate(120, (i) => {'name': 'User $i', 'age': '$i'});
